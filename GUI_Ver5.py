...
def form_metrolst(metro_st, branches):
    metro_st.sort()
    tempLst = []
    i = 0
    while i < len(metro_st):
        if metro_st[i][-2:].isdigit():
            if (i - 1) >= 0 and metro_st[i - 1][:-2] == metro_st[i][:-2]:
                br_num = metro_st[i][-2:]
                tmp_st = metro_st[i][:-2] + ' (' + branches[br_num] + ')'
                tempLst.append(tmp_st)
            else:
                if (i + 1) < len(metro_st) and metro_st[i + 1][:-2] == metro_st[i][:-2]:
                    br_num1 = metro_st[i + 1][-2:]
                    br_num2 = metro_st[i][-2:]
                    tmp_st1 = metro_st[i + 1][:-2] + ' (' + branches[br_num1] + ')'
                    tmp_st2 = metro_st[i][:-2] + ' (' + branches[br_num2] + ')'
                    tempLst.append(tmp_st1)
                    tempLst.append(tmp_st2)
                    i += 1

                else:
                    tempLst.append(metro_st[i][:-2])
        else:
            if (i - 1) >= 0 and metro_st[i - 1][:-1] == metro_st[i][:-1]:
                br_num = metro_st[i][-1]
                tmp_st = metro_st[i][:-1] + ' (' + branches[br_num] + ')'
                tempLst.append(tmp_st)
            else:
                if (i + 1) < len(metro_st) and metro_st[i + 1][:-1] == metro_st[i][:-1]:
                    br_num1 = metro_st[i + 1][-1]
                    br_num2 = metro_st[i][-1]
                    tmp_st1 = metro_st[i + 1][:-1] + ' (' + branches[br_num1] + ')'
                    tmp_st2 = metro_st[i][:-1] + ' (' + branches[br_num2] + ')'
                    tempLst.append(tmp_st1)
                    tempLst.append(tmp_st2)
                    i += 1
                else:
                    tempLst.append(metro_st[i][:-1])
        i += 1
...
revBranchesMos = {branchesMos[i]: i for i in branchesMos.keys()}
revBranchesSpb = {branchesSpb[i]: i for i in branchesSpb.keys()}

metro_Mos = form_metrolst(list(metroTMP), revBranchesMos)
metro_Mos.sort()
metro_Spb = form_metrolst(list(metroTMP1), revBranchesSpb)
metro_Spb.sort()
